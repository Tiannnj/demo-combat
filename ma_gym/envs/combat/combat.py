...
class Combat(gym.Env):
    ...
    def __init_full_obs(self):
        ...
        agent_team_corner = random.randint(0, int(self._grid_shape[0] / 2)), random.randint(0, int(
        ...
        while True:
            pos = random.randint(agent_team_corner[0], self._grid_shape[0] - 5), random.randint(agent_team_corner[1],
                                                                                                self._grid_shape[1] - 5)
            if self._full_obs[pos[0]][pos[1]] == PRE_IDS['empty']:
                opp_team_corner = pos
                break
        ...
        for opp_i in range(self._n_opponents):
            pos = [int(opp_pos_index[agent_i] / 5) + opp_team_corner[0],
                   opp_pos_index[agent_i] % 5 + opp_team_corner[1]]
            while True:
                if self._full_obs[pos[0]][pos[1]] == PRE_IDS['empty']:
    ...
    def render(self, mode='human'):
        ...
        for agent_i in range(self.n_agents):
            if self.agent_health[agent_i] > 0:
                ...
                write_cell_text(img, text=str(self.agent_health[agent_i]), pos=self.agent_pos[agent_i],
                                cell_size=CELL_SIZE,
        ...
        for opp_i in range(self._n_opponents):
            if self.opp_health[opp_i] > 0:
                ...
                write_cell_text(img, text=str(self.opp_health[opp_i]), pos=self.opp_pos[opp_i], cell_size=CELL_SIZE,
                                fill='white', margin=0.3)
    ...
    def step(self, agents_action):
        ...
        for i in range(self.n_agents):
            ...
            print('n_agents', i, rewards[i])
        print('all_reward', rewards)
